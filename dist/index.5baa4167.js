...
const firebaseConfig = {
    ...
    databaseURL: "https://meeting-scheduler-ca23e-default-rtdb.firebaseio.com",
...
function saveMeeting(meeting) {
    const meetingsRef = (0, _database.ref)(database, "meetings");
    (0, _database.push)(meetingsRef, meeting);
}
...
function displayMeeting(meeting) {
    ...
    li.textContent = `${formattedDate} \xe0s ${meeting.time} com Coordenador de ${meeting.teacher}`;
...
function isValidTime(timeString) {
    ...
    const morningStart = 480; // 08:00 in minutes
    const morningEnd = 720; // 12:00 in minutes
    const afternoonStart = 840; // 14:00 in minutes
    const afternoonEnd = 1080; // 18:00 in minutes
...
form.addEventListener("submit", function(event) {
    ...
    if (!isWeekday(date)) {
        alert("Reuni\xf5es somente podem ser agendadas de Segunda \xe0 Sexta.");
        return;
    }
    if (!isValidTime(time)) {
        alert("Reuni\xf5es somente podem ser agendadas entre 08:00-12:00 e 14:00\u201318:00.");
        return;
    }
    const meeting = {
        teacher,
...
function getMeetings() {
    const meetingsRef = (0, _database.ref)(database, "meetings");
    (0, _database.onChildAdded)(meetingsRef, (snapshot)=>{
...
},{}]},["hPIHA","igcvL"], "igcvL", "parcelRequire4487")
